...
def draw(filename, images, labels, boxes, scores, thrh = 0.6):
    for i, im in enumerate(images):
        ...
        while j < len(scores):
            scr = scores[j]
            lab = labels[j][scr > thrh]
            box = boxes[j][scr > thrh]
            
            box = box * np.array([orig_w, orig_h, orig_w, orig_h], dtype=np.float32)
        ...
        im.save(f'results_trt/results_{filename}.jpg')


def process_images_in_folder(folder_path, trt_model, device='cuda:0'):
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.jpg'):
            ...
            blob = {
                'input': im_data.to(device),
                'orig_target_sizes': orig_size.to(device),
            }

            output = trt_model(blob)
            end_time = time.time()
            
            fps = 1 / (end_time - start_time)
            fps_list.append(fps)
            
            print("Fps of this img:",fps)
            class_probs = torch.nn.functional.softmax(output['labels'][:, :, 1:], dim=-1)
            ...
            draw(filename, [im_pil], labels, boxes, scores)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--folder', type=str, required=True)  # Folder of images
    ...
    process_images_in_folder(args.folder, m, device=args.device)
